...
int ComprDataIO::UnpRead(byte *Addr,uint Count)
{
  ...
  while (Count > 0)
  {
    ...
    else
    {
	  ...
	  // 2007/12/12: lundman@lundman.net
	  // Based loosely on patched by elupus at XBMC
	  // Do we want to seek at all?
	  if (m_iSeekTo!=INT64ERR) {
		
		// Do we nede to seek outside this volume? If so, skip to next
		if (m_iSeekTo > CurUnpStart+SrcArc->NewLhd.FullPackSize) {
		  ...
		  // The seek is inside this volume
		  SrcFile->Seek(m_iSeekTo - CurUnpStart,SEEK_CUR);
		  SrcArc->NewLhd.FullUnpSize-=m_iSeekTo;
		  SrcArc->NewLhd.FullPackSize-=(m_iSeekTo - CurUnpStart);
		  UnpPackedSize-=(m_iSeekTo-CurUnpStart);
		  TotalRead = 0;
		  m_iSeekTo = INT64ERR; // clear seek
