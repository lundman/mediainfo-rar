...
#if 0 // lundman
  if (Cmd->FileSizeSeek!=INT64ERR && 
	(Cmd->FileSizeSeek <= Arc.NewLhd.FullPackSize)) {

     if (Arc.NewLhd.Method!=0x30) {
       ErrHandler.Exit(FATAL_ERROR);
     }

     Arc.Seek(Cmd->FileSizeSeek,SEEK_CUR);
     Arc.NewLhd.FullUnpSize-=Cmd->FileSizeSeek;
     Arc.NewLhd.FullPackSize-=Cmd->FileSizeSeek;
  }
#endif

  if (Cmd->FileSizeSeek != INT64ERR) {
	DataIO.m_iSeekTo = Cmd->FileSizeSeek;
	Cmd->FileSizeSeek = INT64ERR;
  }
